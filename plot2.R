...
plot(df$DateTime, df$Global_active_power, xlab=' ', ylab='Global Active Power (kilowatts)', xaxt = "n", type = "l",lwd=2) # formula for plotting  df$Global_active_power versus df$DateTime
at <- seq(1, length(df$weekday), by=1439) # Defining the sequence
labs <- as.character(df$weekday[at])      # creating the custom label annotation
